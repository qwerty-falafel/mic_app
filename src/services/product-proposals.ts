...
export class GptOssProposalAnalyzer implements ProductProposalAnalyzer {
  ...
  async analyse(context: { product: any; brief: any; previous?: any; feedback?: string }) {
    const instruction = `Return JSON only. Act as a product discovery partner. Turn the Brief into a conservative proposal with this shape: {"rationale":"", "features":[{"name":"","description":""}], "epics":[{"name":"","outcome":"","featureNames":[]}], "items":[{"kind":"story|defect|discovery","title":"","value":"","acceptanceCriteria":[],"featureName":"","epicName":""}], "dependencies":[], "uncertainties":[]}. A Story delivers user value; a Defect removes a barrier to intended value; a Discovery closes a consequential understanding gap. Do not create implementation tasks. Product: ${JSON.stringify(context.product)}. Brief: ${JSON.stringify(context.brief)}. Previous proposal: ${JSON.stringify(context.previous ?? null)}. Revision feedback: ${context.feedback ?? 'none'}.`;
...
export class ProductProposalService {
  ...
  async decide(proposalId: string, kind: 'accepted' | 'revision-requested' | 'rejected', actor: string, feedback?: string) {
    ...
    return this.db.transaction(async tx => {
      ...
      if (!brief) throw new Error('Brief not found');
      if (kind === 'accepted') await this.apply(tx as any, brief.projectId, proposal.proposal as ProposalShape);
      await tx.update(productBriefs).set({ status: kind, updatedAt: new Date() }).where(eq(productBriefs.id, brief.id));
      return decision!;
    });
  }
  private async apply(tx: Database, projectId: string, proposal: ProposalShape) {
    ...
    for (const candidate of proposal.epics ?? []) {
      ...
      if (!row) { const identity = randomUUID(); [row] = await tx.insert(productEpics).values({ id: `epic_${identity}`, projectId, slug: durableSlug(candidate.name, identity), name: candidate.name, outcome: candidate.outcome ?? '', status: 'proposed' }).returning(); }
