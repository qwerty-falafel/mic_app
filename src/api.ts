...
export function createApp(db: Database, executor?: LifecycleExecutor, proposalAnalyzer?: ProductProposalAnalyzer) {
  ...
  app.get('/products/:reference/roadmap', async (request, reply) => { const { reference } = z.object({ reference: id }).parse(request.params); const product = await products.getByReference(reference); return product ? products.roadmap(product.id) : reply.code(404).send({ error: 'not_found' }); });
  ...
  app.get('/products/:id/activity', async request => {
    const { id: projectId } = z.object({ id }).parse(request.params);
