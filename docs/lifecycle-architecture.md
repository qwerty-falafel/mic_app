...
stateDiagram-v2
    ...
    AWAITING_PLANNING_APPROVAL --> PLANNING_GRADE: reviewer requests revision
...
Planning and implementation artifact rows store both baseline and result Git revisions plus a content hash. Planning feedback names the reviewed artifact hash and is stored in the audit timeline. The next planning run receives that feedback, reuses the existing isolated worktree and updates its planning artifacts in place. Each revision produces a new run and content hash; planning approval is accepted only for the latest artifact. Approval rows bind the approver to that hash and result revision. The implementation approval fast-forwards the repository to the approved result and removes the worktree. A non-fast-forward is rejected so concurrent base changes cannot be silently overwritten.
