...
6. Model Sprint Goal, selected backlog items, status, and Increment only if explicit Sprint records are enabled; do not infer a Scrum Sprint merely from an epic story list.
...
11. Present Feature health, Epic progress, and Sprint/tracking progress separately so the concepts cannot be confused.
