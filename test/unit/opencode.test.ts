...
describe('OpenCodeAdapter', () => {
  let root = '', originalPath = process.env.PATH;
  afterEach(async () => { process.env.PATH = originalPath; if (root) await rm(root, { recursive: true, force: true }); });
  ...
  it('cancels a child and recovers its persisted terminal observation', async () => {
    ...
    expect((await adapter.observe('cancel-run', true)).status).toBe('cancelled');
